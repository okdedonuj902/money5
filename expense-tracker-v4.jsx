...
const SAMPLE = [
  { id: uid(), date: "2026-04-04", item: "拿鐵咖啡",    note: "下午工作",   catMain: "food",    catSub: "food-cafe",  payment: "cash",     amount: 160 },
  { id: uid(), date: "2026-04-03", item: "捷運月票",    note: "",          catMain: "transport",catSub: "tp-mrt",     payment: "card",     amount: 1280 },
  { id: uid(), date: "2026-04-02", item: "《島嶼來的孩子》",note:"誠品購入", catMain: "education",catSub: "ed-book",    payment: "card",     amount: 380 },
  { id: uid(), date: "2026-04-01", item: "家庭晚餐",    note: "壽喜燒",    catMain: "food",    catSub: "food-eat",   payment: "transfer", amount: 1200 },
];
...
function SettingsTab({ categories, setCategories, calcIcon, setCalcIcon }) {
  ...
  const [editMain, setEditMain] = useState(null); // id or "new"
  const [editSub,  setEditSub]  = useState(null); // { mainId, subId } or { mainId, subId:"new" }
...
export default function App() {
  ...
  function submitForm() {
    ...
    setRecords(p=>[{...form,id:uid(),amount:+form.amount},...p]);
                  ...
                  <div key={r.id} style={cardSt}>
                    <div style={{ display:"flex",alignItems:"flex-start",gap:12 }}>
                      ...
                      <div style={{ display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4 }}>
                        ...
                        <button onClick={()=>setRecords(p=>p.filter(x=>x.id!==r.id))} style={{ fontSize:10,color:P.border,background:"none",border:"none",cursor:"pointer",padding:0 }}>刪除</button>
                      </div>
                    </div>
                  </div>
                );
            ...
            <div style={{ marginBottom:13 }}>
              ...
              <div style={{ display:"flex",gap:8 }}>
                ...
                <button onClick={()=>setShowCalc(true)}
                  style={{ padding:"0 13px",background:P.accentLight,color:P.accent,border:`1.5px solid ${P.accent}44`,borderRadius:11,cursor:"pointer",fontSize:22,flexShrink:0,overflow:"hidden",width:50,height:50,display:"flex",alignItems:"center",justifyContent:"center" }}>
                  {calcIcon.img
