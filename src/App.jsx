...
function CatThumb({ item, size = 20, box = 36, accentLight }) {
  return (
    <div style={{ width:box, height:box, borderRadius:box*0.28, background:accentLight||"#EBF3EE", display:"flex", alignItems:"center", justifyContent:"center", fontSize:size, flexShrink:0, overflow:"hidden" }}>
...
function Calculator({ initial="", theme, onConfirm, onClose }) {
  ...
  function bc(v) {
    ...
    if ("C⌫".includes(v))  return { bg:"#fde8e8",   fg:"#e07070" };
...
function SettingsTab({ categories, setCategories, calcIcon, setCalcIcon, theme }) {
  ...
  return (
    <div>
        ...
        <div>
          ...
          <div style={{ background:T.card,borderRadius:14,padding:"14px",boxShadow:"0 1px 4px rgba(0,0,0,0.04)",display:"flex",alignItems:"center",gap:12 }}>
            ...
            <div style={{ flex:1 }}>
              ...
              <input ref={calcImgRef} type="file" accept="image/*" style={{ display:"none" }} onChange={e=>{ const f=e.target.files?.[0]; if(f) imgReader(f,d=>setCalcIcon({emoji:"",img:d})); }} />
            </div>
          </div>
        </div>
      )}
      ...
      {section==="cats" && (
        <div>
          <div style={{ fontSize:13,fontWeight:700,color:T.ink,marginBottom:10 }}>管理分類</div>
          {categories.map(cat=>(
              ...
              {editMain===cat.id ? (
                <div style={{ padding:"12px 14px" }}>
                  ...
                  <div style={{ display:"flex",gap:8 }}>
                    <button onClick={saveMain} style={{ ...bSt(T.accent,"#fff"),flex:1 }}>儲存</button>
                ...
                <div style={{ padding:"11px 14px",display:"flex",alignItems:"center",gap:10,cursor:"pointer" }} onClick={()=>setExpandedMain(v=>v===cat.id?null:cat.id)}>
                  <CatThumb item={cat} size={18} box={34} accentLight={T.accentLight} />
                    ...
                    <div key={sub.id}>
                      {editSub?.mainId===cat.id && editSub?.subId===sub.id ? (
                        <div style={{ padding:"10px 14px 10px 50px" }}>
                          ...
                          <div style={{ display:"flex",gap:8 }}>
                            <button onClick={saveSub} style={{ ...bSt(T.accent,"#fff"),flex:1 }}>儲存</button>
          ...
          {editMain==="new" ? (
            ...
            <button onClick={()=>{ setEditMain("new"); setDraftMain({label:"",icon:"",img:null}); }}
              style={{ width:"100%",padding:13,background:"none",color:T.accent,border:`1.5px solid ${T.accent}`,borderRadius:14,fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit" }}>
              ＋ 新增大分類
            </button>
          )}
        </div>
    ...
    <div style={{ fontFamily:"'Noto Serif TC','Noto Sans TC',serif",background:T.bg,minHeight:"100vh",display:"flex",justifyContent:"center",padding:"0 0 48px" }}>
      ...
      <div style={{ width:"100%",maxWidth:420 }}>
        ...
        <div style={{ background:T.headerBg,borderBottom:`1px solid ${T.border}`,padding:"18px 18px 0" }}>
          ...
          <div style={{ display:"flex",borderTop:`1px solid ${T.border}` }}>
            {[["home","明細"],["stats","統計"],["settings","設定"]].map(([k,l])=>(
              <button key={k} onClick={()=>setTab(k)} style={{ flex:1,padding:"11px 0",border:"none",background:"none",cursor:"pointer",fontSize:12,fontWeight:tab===k?700:500,color:tab===k?T.accent:T.muted,borderBottom:tab===k?`2px solid ${T.accent}`:"2px solid transparent",transition:"all 0.15s",fontFamily:"inherit" }}>{l}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ padding:16 }}>
          ...
          {tab==="home" && (
            <>
              {filtered.length===0 && <div style={{ textAlign:"center",color:T.muted,padding:"48px 0",fontSize:14 }}><div style={{ fontSize:32,marginBottom:10 }}>🌿</div>這個月還沒有記錄</div>}
              {filtered.map(r=>{
                ...
                return (
                  <div key={r.id} style={cardSt}>
                    <div style={{ display:"flex",alignItems:"flex-start",gap:12 }}>
                      <CatThumb item={sub||main} accentLight={T.accentLight} />
                ...
                <div key={c.id} style={{ ...cardSt,padding:"12px 16px" }}>
                  <div style={{ display:"flex",justifyContent:"space-between",marginBottom:7,alignItems:"center" }}>
                    <div style={{ display:"flex",gap:8,alignItems:"center" }}>
                      <CatThumb item={c} size={16} box={30} accentLight={T.accentLight} />
      ...
      {showForm && (
        <div style={{ position:"fixed",inset:0,background:"rgba(44,44,44,0.4)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:999,backdropFilter:"blur(3px)" }}
          ...
          <div style={{ background:T.card,borderRadius:"24px 24px 0 0",padding:"22px 18px 32px",width:"100%",maxWidth:420,maxHeight:"92vh",overflowY:"auto" }}>
            ...
            {formError&&<div style={{ fontSize:12,color:"#e07070",marginBottom:10,textAlign:"center" }}>{formError}</div>}
            <button onClick={submitForm} style={{ width:"100%",padding:15,background:T.accent,color:"#fff",border:"none",borderRadius:14,fontSize:15,fontWeight:700,cursor:"pointer",letterSpacing:1,fontFamily:"inherit" }}>
              儲存記錄
            </button>
          </div>
        </div>
